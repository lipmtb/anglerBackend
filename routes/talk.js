...
router.get("/collectTalk", function (req, res) {
    ...
    console.log("钓友圈发布者ID", req.query.anglerId);
    if (!req.query.anglerId) {
        console.log("发布者信息不存在");
        res.send({
            errMsg: 0,
            loginErr: 1
    ...
    collecting.anglerId = req.query.anglerId;
...
router.get("/cancelCollectTalk", function (req, res) {
    ...
    console.log("收藏的Id", req.query.collectId);
    if (!req.query.collectId) {
        console.log("收藏信息不存在");
        res.send({
            errMsg: 0
        })
